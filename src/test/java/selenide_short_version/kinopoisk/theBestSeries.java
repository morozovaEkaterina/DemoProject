...
import selenide_short_version.kinopoisk.*;
...
public class theBestSeries extends BaseTest {
    ...
    public void check() {
        ...
        tabPage(Selenide.webdriver().driver().getWebDriver());
        ...
        kinoPoiskMain.seriesBtn.should(exist, clickable, visible).click();
        Assertions.assertEquals(SeriesPage.URL, Selenide.webdriver().driver().url());
        seriesPage.collectionOfLists.get(1).should(exist, clickable, visible).click();
        top250Series.fullListOfSeriesBtn.should(exist, visible, clickable);
